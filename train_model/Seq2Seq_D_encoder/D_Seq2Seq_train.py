...
from train_model.Seq2Seq_D_encoder import D_get_dataloader
from train_model.Seq2Seq_D_encoder.D_Seq2Seq_dataset import *
from train_model.Seq2Seq_D_encoder.D_Seq2Seq_model import *
...
loss_criterion = nn.CrossEntropyLoss(weight = weight_mask).to(
        device)  # 定义loss，多分类损失。CrossEntropyLoss内部带有softmax，所以模型不需要做softmax

try:
    model = save_load_model.load('Seq2Seq_D_encoder')
    model.train()
    print('Load Model OK!')
except:
...
dataloader = D_get_dataloader.get_DataLoader(D_list, word2index)
...
for epoch in range(0, 10):  # epoch循环
    ...
    for item in dataloader:
        ...
        if ((cnt + 1) % 20 == 0):
            save_load_model.save(model, 'Seq2Seq_D_encoder')

        torch.cuda.empty_cache()

        cnt += 1
        time_end = time.time()
        print('epoch:{},cnt:{},loss:{}'.format(epoch, cnt, loss.data.item()))
        if ((cnt + 1) % 4 == 0):
            plot(loss.data.item())
